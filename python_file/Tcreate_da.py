...
def input_vtx():
    ...
    data = np.zeros(((int(len(row)/10), 8, 2))) #各断面ごとの頂点座標を格納
    G_data = np.zeros((int(len(row)/10), 3)) #三次元中心データの格納
    V_data = np.zeros((int(len(row)/10), 3)) #要素座標ベクトルの格納(SEAMFEM座標系では[y, z, x]で格納)
 
    for i in range(int(len(row)/10)):
        rowdata = row[10*i].replace("\n", "")
        rowdata = rowdata.split(" ")
        G_data[i] = np.array([float(rowdata[0]), float(rowdata[1]), float(rowdata[2])])

        rowdata = row[10*i + 1].replace("\n", "")
        rowdata = rowdata.split(" ")
        V_data[i] = np.array([float(rowdata[0]), float(rowdata[1]), float(rowdata[2])])

        for j in range(8):
            rowdata = row[10*i + j + 2].replace("\n", "")
            rowdata = rowdata.split(" ")
...
def main():
    ...
    # 座標系を各web中心点（0,0）に合わせる
    for i in range(len(data)):
        mean = np.array([np.mean(data[i][4:].T[0]), np.mean(data[i][4:].T[1])])
    ...
    for i in range(len(data)):
        ...
        lw_mean = np.array([np.mean(data[i][[4, 6]].T[0]), np.mean(data[i][[4, 6]].T[1])])
        rw_mean = np.array([np.mean(data[i][[5, 7]].T[0]), np.mean(data[i][[5, 7]].T[1])])
        ...
        lw_lig = np.linalg.norm(data[i][4] - data[i][6])
        rw_lig = np.linalg.norm(data[i][5] - data[i][7])
    ...
    if CS==0 or CS==1:
        argm_data = np.zeros((2, 6))
        argm_data = np.array([uf, w])
        
        nn = np.zeros((2, 2)) #分割数を格納するための配列(0: 厚さ、1:幅)
        ...
        argms_data = np.zeros((len(data), 2, 6))
        for i in range(len(data)):
            argms_data[i] = np.array([uflange[i], web[i]])

        nnn = np.zeros(((len(data), 2, 2))) #分割数を格納するための配列(0: 各断面, 1: 厚さ、2:幅)
    ...
    if EDIT == "":
        if CS==0 or CS==1:
            ...
            with open(DIR + "create.da_", "w") as f:
                ...
                print("{:>10d}{:>10d}{:>10d}{:>10d}{:>15}{:>10}{:>10f}{:>10f}{:>10f}{:>10d}{:>10d}{:>10d}{:>10d}{:>10d}{:>10d}".format(1, 1, 2, 0, "", "", V_m[2], V_m[0], V_m[1], 0, 0, 0, 0, 0, 0), file = f)
            ...
            with open(DIR + "create_dif.da_", "w") as f:
                ...
                for i in range(len(data)):
                    print("/sections", file=f)
                    print("{:>10d}{:>10d}{:>10d}{:>10d}{:>15}{:>10}{:>10f}{:>10f}{:>10f}{:>10d}{:>10d}{:>10d}{:>10d}{:>10d}{:>10d}".format(i+1, 1, 2, 0, "", "", V_data[i][2], V_data[i][0], V_data[i][1], 0, 0, 0, 0, 0, 0), file = f)
                    for j in range(2):
